...
export class Inventory {
    ...
    public setClothes(player: PlayerMp, slotnumber: number, data: { component: number; drawable: number; texture: number }): void {
        ...
        const itemData = this.items.clothes[slotnumber];
        if (!itemData) return;
    ...
    public reloadClothes(player: PlayerMp): void {
        Object.entries(this.items.clothes).forEach(([index, clothing]) => {
            ...
            const parsedKey = clothingKey ? tryParse(clothingKey) : null;
        ...
        for (let i = 0; i <= 13; i++) {
            if (this.items.clothes[i]) {
                ...
                if (playerClothes && playerClothes.key && playerClothes.isPlaced && playerClothes.type !== null) {
                    ...
                    this.loadClothes(player, i, tryParse(data));
                }
            } else {
                this.removeClothes(player, i);
            }
    ...
    setInventory(player: PlayerMp): void {
        try {
            ...
            player.call("client::eventManager", ["cef::inventory:setInventory", JSON.stringify(data)]);
            player.call("client::eventManager", ["cef::inventory:setQuickUseItems", JSON.stringify(this.quickUse)]);
            ...
            player.call("client::eventManager", ["cef::inventory:setDroppedItems", JSON.stringify(groundItems)]);
    ...
    public deleteItemStack(player: PlayerMp, data: string): void {
        ...
        try {
            ...
            const itemData = this.items[source.component][source.slot];
            if (!itemData) return;

            const count = itemData.count;

            if (count > 1) {
                this.items[source.component][source.slot] = { ...item, count: count - 1 };
                this.setInventory(player);
            } else {
                const fastSlotIndex = Object.values(this.quickUse).findIndex((e) => e && e.component === source.component && tryParse(e.id) === source.slot);
                if (fastSlotIndex !== -1) {
                    this.quickUse[fastSlotIndex] = null;
                }

                this.deleteItem(item.hash);
            }
            ...
            for (const category in items) {
                if (Object.prototype.hasOwnProperty.call(items, category)) {
                    ...
                    for (const [slot, item] of Object.entries(categoryItems)) {
                        ...
                        if (item.hash === uuid) {
                            const parsedSlot = tryParse(slot);
                            const fastSlotIndex = Object.values(quickUse).findIndex((e) => e && e.component === category && tryParse(e.id) === parsedSlot);
